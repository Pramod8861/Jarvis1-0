...
async function initJARVIS() {
    console.log("Initializing JARVIS..."); // Debugging
    await speak("Activating JARVIS..");
    await wishMe();
}

window.addEventListener('load', () => {
    initJARVIS().catch(error => console.error('Error initializing JARVIS:', error));
});
...
async function casualConversation() {
    ...
    recognition.onresult = async (event) => {
        ...
        console.log("Casual conversation transcript:", transcript); // Debugging
        await saveInteraction('User', transcript);
        addToHistory('User', transcript);
